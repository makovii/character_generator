...
export class AuthService {
  ...
  async login(userDto: CreateUserDto) {
    ...
    await this.jwtRepository.create({ userId: user.id, jwt: token, isActive: true, tokenId: uuid });
  ...
  async logout(userDto: CreateUserDto) {
    ...
    try {
      await this.jwtRepository.update({ isActive: false }, { where: { userId: user.id }});
  ...
  async registration(userDto: CreateUserDto) {
    ...
    if (dto.email) {
      ...
      const link = `${env.get('LINK_HOME_PAGE').required().asString()}/auth/confirmEmail/${token}`;
      ...
      const link = `${env.get('LINK_HOME_PAGE').required().asString()}/auth/confirmPhone`;
      const code = Array(5)
      .fill(null)
      .map(() => Math.floor(Math.random() * 10))
      .join('');

      await sendSms(env.get('FROM_SMS').required().asString(), dto.phone, `Your code- ${code} .`);
      await this.userService.createCandidate({ ...dto, code });
      return link
    }
  }

  private async generateToken(user: GenerateToken) {
    const payload = {
      id: user.id,
      name: user.name,
      email: user.email,
  ...
  async checkIsActive(tokenId: string) {
    if (!tokenId) return false;
    const jwt = await this.jwtRepository.findOne({attributes: ['isActive'], where: { tokenId }});
    if(jwt === null ) return false;
    return jwt.isActive;
  }

  async getLastJwtDateByUserId(id: number) {
    return await this.jwtRepository.findOne({attributes: ['tokenId', 'createdAt'], where: { userId: id }, order: [['createdAt', 'DESC']]});
  }
}
